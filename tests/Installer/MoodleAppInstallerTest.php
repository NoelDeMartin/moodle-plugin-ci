...
class MoodleAppInstallerTest extends MoodleTestCase
{
    public function testInstall()
    {
        ...
        $this->assertMatchesRegularExpression('/docker/', $execute->cmds[0]);
        $this->assertMatchesRegularExpression('/run/', $execute->cmds[0]);
        $this->assertMatchesRegularExpression('/moodlehq\/moodleapp:latest-test/', $execute->cmds[0]);

        $this->assertMatchesRegularExpression('/git/', $execute->cmds[1]);
        $this->assertMatchesRegularExpression('/clone/', $execute->cmds[1]);
        $this->assertMatchesRegularExpression('/https:\/\/github\.com\/moodlehq\/moodle-local_moodleappbehat\.git/', $execute->cmds[1]);
    }
}
